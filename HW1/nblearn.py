...
# stopWordsSet = {'is', 'under', 'about', 'will', 'other', 'these', 'hers', 'nor', 'did', 'won', 'but', 't', 'my', 'they', 'own', 'by', 'me', 'same', 'hasn', 'just', 'so', 'to', 'how', 'not', 'm', 'wouldn', 'yourself', 'herself', 'being', 'he', 'am', 'again', 'each', 'than', 'against', 'some', 'during', 'over', 'your', 'himself', 'haven', 'because', 'has', 'those', 'any', 'and', 'before', 'don', 'were', 'o', 'above', 'myself', 'this', 'doing', 'its', 've', 'should', 'll', 'then', 'having', 'had', 'here', 'such', 'weren', 'ma', 'shouldn', 'been', 'have', 'does', 'the', 'or', 'why', 'very', 'on', 'him',
#                 'in', 'while', 'd', 'after', 'now', 'no', 'most', 'all', 's', 'shan', 'as', 'their', 'if', 'an', 'be', 'do', 'down', 'her', 'are', 'was', 'more', 'it', 'with', 'there', 'y', 'which', 'once', 'at', 'off', 'wasn', 'theirs', 'below', 'a', 'couldn', 'yours', 'themselves', 're', 'mightn', 'mustn', 'when', 'between', 'doesn', 'didn', 'ours', 'i', 'that', 'what', 'both', 'yourselves', 'through', 'who', 'aren', 'needn', 'into', 'for', 'up', 'until', 'whom', 'itself', 'from', 'ourselves', 'she', 'further', 'too', 'you', 'his', 'we', 'only', 'them', 'ain', 'where', 'out', 'isn', 'our', 'of', 'hadn', 'few', 'can'}
...
def learnModel(**kwargs):
    ...
    logPrior = {}
    # Prior[SPAM_CATEGORY] = no_of_spam_emails/total_no_of_emails
    # Prior[HAM_CATEGORY] = no_of_ham_emails/total_no_of_emails
    ...
    for word in vocabulary:
        ...
        # likelihood[SPAM_CATEGORY][word] = (noOfOccurences + 1)/denomitorSPAM               # add_one smoothing
        ...
        # likelihood[HAM_CATEGORY][word] = (noOfOccurences + 1)/denomitorHAM                # add_one smoothing
    ...
    results = {}
    # converting set to list, as set cannot be written to json
    results["vocabulary"] = list(vocabulary)
...
def learnAllLabelData(trainingPath, **kwargs):
    ...
    if kwargs["ignoreCharToken"] or kwargs["binaryNB"] or kwargs["ignoreStopWords"]:
        # init singleTokenVocab if one of modification rule is enabled, so as to pass data structure
        singleTokenVocab = {}
        stopWordsVocab = {}
    ...
    if kwargs["threshold"]:
        try:
            ...
            removeFromHam = False
            print("Before:")
            print("reset vocabulary and bagofWords to meet threshold limits : {}".format(
                kwargs["threshold"]))
            print("len(bagOfWords[SPAM_CATEGORY].keys()) : ", len(
                bagOfWords[SPAM_CATEGORY].keys()))
            print("len(bagOfWords[HAM_CATEGORY].keys()) : ", len(
                bagOfWords[HAM_CATEGORY].keys()))
...
def main():
    parser = argparse.ArgumentParser(
        description="learn a Naive Bayes model from labeled data")
    parser.add_argument("-p", "--percentage",
                        help="percentage of labelled data to learn from", type=int)
    parser.add_argument("-v", "--verbose",
                        help="increase output verbosity", action="store_true")
    parser.add_argument("-i", "--ignoreChar",
                        help="ignore single character as tokens", action="store_true")
    parser.add_argument("-s", "--ignoreStopWords",
                        help="ignore Stop Words from data", action="store_true")
    parser.add_argument("-b", "--binaryNB",
                        help="Multi-variate Bernoulli model : based on binary data", action="store_true")
    parser.add_argument("-t", "--thresholdWords",
                        help="set threshold level for frequency words to be processed", type=int)
    ...
    if args.percentage:
        ...
        learnPartLabelData(trainingPath, verbose=args.verbose,
                           percentScan=args.percentage)
    else:
        if args.verbose:
        ...
        learnAllLabelData(trainingPath, verbose=args.verbose, ignoreCharToken=ignoreCharToken,
                          binaryNB=binaryNB, ignoreStopWords=ignoreStopWords, threshold=threshold)
