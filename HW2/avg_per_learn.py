...
def trainModel(**kwargs):
    global bias, weightVocabulary, avg_bias, avg_weightVocabulary, count
    for i in range(noOfTrainingIterations):
        print("Avg Perceptron Training Iteration {}".format(i))
        if kwargs["verbose"]:
            print("Iteration {} : bias = {} avg_bias = {}".format(i, bias, avg_bias))
            print("weight {}".format(weightVocabulary))
            print("avg_weight {}".format(avg_weightVocabulary))
        ...
        for emailFile in emailFileNames:
            ...
            if (y * activation) <= 0 :                
                ...
                #    avg_weightVocabulary[feature] = avg_weightVocabulary[feature] + y*count*allTrainingEmails[emailFile]["data"][feature]
                emailFeatures = allTrainingEmails[emailFile]["data"].keys()
                for feature in emailFeatures:
                    weightVocabulary[feature] = weightVocabulary[feature] + y*allTrainingEmails[emailFile]["data"][feature]
                    avg_weightVocabulary[feature] = avg_weightVocabulary[feature] + y*count*allTrainingEmails[emailFile]["data"][feature]
                bias = bias + y
                avg_bias = avg_bias + y*count
                if kwargs["verbose"]:
                    print("\t   ",weightVocabulary, bias)
                    print("\tAvg",avg_weightVocabulary, avg_bias)
            count += 1
    for feature in weightVocabulary.keys():
        avg_weightVocabulary[feature] = weightVocabulary[feature] - ((1/count)*avg_weightVocabulary[feature])
    avg_bias = bias - ((1/count)*avg_bias)
    
    if kwargs["verbose"]:
        print()
        for feature in weightVocabulary.keys():
            print("{} {} {}".format(feature, weightVocabulary[feature], round(avg_weightVocabulary[feature],2) ))
        print()
        print("bias {} avg_bias {}\n".format(bias, avg_bias)) 
        print()
        print("Final Model Parameters")
        print("avg_weights {} \navg_bias {}\n".format(avg_weightVocabulary, avg_bias))
    
    print("Avg Perceptron Model Trained. Parameters loaded into per_model.txt")
    results = {}
    results["weights"] = avg_weightVocabulary
    results["bias"] = avg_bias
...
def main():
    parser = argparse.ArgumentParser(description="learn a perceptron model from labeled data using the averaged perceptron training algorithm")
